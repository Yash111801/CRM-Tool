...
const getStudyByDivision = async (req, res) => {
  try {
    ...
    const studyData = await Study.find(query).select('_id title studyId').sort({ title: 1 });
    return res.json({
      status: 200,
      data: studyData,
      message:"Study list fetched successfully."
...
const getDashboardData = async (req, res) => {
  try {
    ...
    const columnChartData = await getMonthlyStatusData({studyId, selectedYear});
...
const getDashboardColumnChartData = async (req, res) => {
  try {
    ...
    return res.json({
      status: 201,
      message: error.message || "Internal server error while get dashboard column chart data.",
    });
  }
}

async function getMonthlyStatusData({ studyId, selectedYear, statuses = ["completed", "readyToCreate", "discrimination"] }) {
  const pipeline = [
    {
      $match: {
        study: new ObjectId(studyId),
        createdAt: {
          $gte: new Date(`${selectedYear}-04-01T00:00:00.000Z`),
          $lte: new Date(`${selectedYear}-04-30T23:59:59.999Z`),
        }
      }
    },
    {
...
const generateReport = async (req, res) => {
  try {
    ...
    const studyData = await StudyExcelData.find(query).select("-status -deletedAt -createdAt -updatedAt -__v").lean();
    res.json({
      status: 200,
      data: studyData,
